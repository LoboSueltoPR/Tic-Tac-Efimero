// Polling Green API → Groq → Google Calendar
// Llamado cada minuto por cron-job.org (GET /api/poll)
...
// ── Green API polling ─────────────────────────────────────────────────────────
const BASE     = `https://7107.api.greenapi.com/waInstance${process.env.GREEN_ID_INSTANCE}`;
const TOKEN    = process.env.GREEN_API_TOKEN;
const OWNER_ID = `${process.env.OWNER_PHONE}@c.us`;

async function gaSend(text) {
  const res = await fetch(`${BASE}/sendMessage/${TOKEN}`, {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ chatId: OWNER_ID, message: text }),
  });
...
async function deleteOne(receiptId) {
  await fetch(`${BASE}/deleteNotification/${TOKEN}/${receiptId}`, { method: 'DELETE' });
}
...
export default async function handler(req, res) {
  ...
  try {
    for (let i = 0; i < 20; i++) {
      const notification = await receiveOne();
      if (!notification?.receiptId) break;

      const { receiptId, body } = notification;
      const { typeWebhook, senderData, messageData } = body ?? {};

      console.log('POLL typeWebhook:', typeWebhook, '| from:', senderData?.chatId);

      const isSelfMessage =
        (typeWebhook === 'incomingMessageReceived' || typeWebhook === 'outgoingMessageReceived') &&
        messageData?.typeMessage === 'textMessage' &&
        senderData?.chatId === OWNER_ID;
